...
func (r *paymentRepo) GetByIdempotencyKey(ctx context.Context, key string) (*model.Payment, error) {
	var p model.Payment
	err := r.db.QueryRow(ctx,
		`SELECT id, invoice_id, amount, status, method, qr_code, idempotency_key, updated_at FROM payments WHERE idempotency_key=$1`, key,
	).Scan(&p.ID, &p.InvoiceID, &p.Amount, &p.Status, &p.Method, &p.QRCode, &p.IdempotencyKey, &p.UpdatedAt)
	if err != nil {
		return nil, err
	}
